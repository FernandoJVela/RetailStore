...
public class Order : AggregateRoot
{
    ...
    {
        if (!_items.Any())
            throw new DomainException(OrderErrors.CannotConfirmEmptyOrder());
    ...
    public void MarkShipped()
    {
        if (Status != OrderStatus.Confirmed)
            throw new DomainException(
                OrderErrors.InvalidOrderStatusTransition(Status, OrderStatus.Shipped));
    ...
    public void MarkDelivered()
    {
        if (Status != OrderStatus.Shipped)
            throw new DomainException(
                OrderErrors.InvalidOrderStatusTransition(Status, OrderStatus.Delivered));

        Status = OrderStatus.Delivered;
        Touch();
        Raise(new OrderDeliveredEvent(Id, CustomerId));
    }
    ...
    public void Cancel(string reason)
    {
        ...
        if (Status == OrderStatus.Cancelled)
            throw new DomainException(OrderErrors.OrderAlreadyCancelled());
